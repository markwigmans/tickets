package com.chessix.tickets.ap.controller;
...
import com.chessix.tickets.ap.model.BoxOffice;
...
public class TicketController {
    ...
    @ResponseBody
    public EventCreated createEvent(@PathVariable final String event, @PathVariable final int tickets) {
    ...
    @ResponseBody
    public Callable<AbstractTicketMessage> buySingle(@PathVariable final String event) {
    ...
    @ResponseBody
    public Callable<AbstractTicketMessage> buyMultiple(@PathVariable final String event, @PathVariable final int tickets) {
        return new Callable<AbstractTicketMessage>() {
            public AbstractTicketMessage call() throws Exception {
                logger.debug("buy({},{})", event, tickets);
                final BoxOffice manager = eventManagers.get(EventUtils.getEventName(event));
                if (manager != null) {
                    return manager.buy(tickets);
                }
                throw new IllegalArgumentException(String.format("Unknown event: '%s'", event));
            }
        };
    }
}
